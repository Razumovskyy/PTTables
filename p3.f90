...
subroutine LBL2016(GASCOMP, IGAS, VS1)
    ...
    save TST,EK,NTAB_G,T_G1,C_G2,C_G1
...
! -------------------------------------------------------------------------- !
	...
	IF(P /= P_P .OR. T /= T_T .OR. RO /= RO_RO) THEN
 		IF(T /= T_T)THEN
  			T=T_T ; TT=T ; TST = TS/T ; EK = BET*(T-TS)/(T*TS)
...
!###	COMMON/FILES/FNAME(NMOL),NZ(NMOL)
	COMMON/FILES/NZ(NMOL),FNAME(NMOL)	
		DO 100 K = 1, NMOL
!*
	IF(MOLECULE == COMPONENT(K))THEN
!### FNAM_='C:\HITRAN16\'//FNAME(K)
FNAM_='./HITRAN/'//FNAME(K)
		OPEN(IDENT, ACCESS='DIRECT', FORM='UNFORMATTED',	&
			RECL=36, FILE=FNAM_)! 36 for LINUX
	NZCOMP	= K
	GO TO 10
								ENDIF
!*
...
!*
	...
	WRITE(*,*)'(par. NLIN in SEARC2000,CATAL2016,LBL2016 should be increased)'
		PAUSE
		END IF
...
!# 'COF2','SF6','H2S','HCOOH','HO2','O','ClONO2','NO+','HOBr', &
   'COF2','SF6','H2S','HCOOH','HO2','O','ClONO','NO+','HOBr', &
'C2H4','CH3OH','CH3Br','CH3CN','CF4', &
